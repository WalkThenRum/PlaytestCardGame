if(active){
	if(deckShuffled){
		for(var i=0;i<handSize;i++){
			if(cardMana[i]>mana){
				TextButton2((.25+.1*i)*SCREENWIDTH,.9*SCREENHEIGHT,(.35+.1*i)*SCREENWIDTH-1,SCREENHEIGHT-1,cardName[i]);
			}else if(TextButton((.25+.1*i)*SCREENWIDTH,.9*SCREENHEIGHT,(.35+.1*i)*SCREENWIDTH-1,SCREENHEIGHT-1,cardName[i])){
				if(!placingUnit){
					mana-=cardMana[i];
